...
app.post('/api/slides/animate', async (req, res) => {
  ...
  try {
    ...
    console.log('Slides v8.56.0:', slides.length, (videoType||'slides'), W+'x'+H, audioMode||'music', 'stock='+(stockMode||'none'), 'pythonReady='+pythonReady);
...
`);
    ...
    const fileSize = fs.statSync(outputPath).size;
    outputStore[videoId] = { path:outputPath, size:fileSize, created:Date.now() };
    console.log('Slides v8.56.0 ready:', fileSize, 'bytes, id:', videoId);
    ...
    console.error('Slides v8.56.0 error:', e.message);
    res.status(500).json({ error: e.message });
  } finally {
    try { fs.rmSync(tempDir,{recursive:true,force:true}); } catch(e) {}
  }
...
app.listen(PORT, function() {
  console.log('EnerStudio Backend v8.56.0 running on port ' + PORT);
