...
app.get('/api/keepalive', async (req, res) => {
  try {
    // Run a REAL (tiny) database query — health endpoints don't count as activity
    // for Supabase's auto-pause logic, but a PostgREST query does. RLS means the
    // anon key gets an empty result; the query itself is what keeps the project active.
    ...
    // Report FAILURE with a 500 so UptimeRobot turns red instead of staying green
    res.status(500).json({ ok: false, error: String(e && e.message || e), t: Date.now() });
  }
});

// ── SECURITY: require a valid logged-in Supabase member (protects money-burning endpoints) ──
// The app auto-attaches the member's Supabase access token; here we verify it with Supabase.
async function requireMember(req, res, next) {
  try {
    const h = req.headers['authorization'] || '';
    const token = h.startsWith('Bearer ') ? h.slice(7) : '';
    if (!token) return res.status(401).json({ error: 'Please log in to use this feature.' });
    let r;
    try {
      r = await fetch(SUPABASE_URL_KA + '/auth/v1/user', {
...
// ── SECURITY: simple per-IP rate limiter (in-memory; Render free tier is single-instance) ──
const _rlBuckets = {};
function rateLimit(maxPerMinute) {
  return function (req, res, next) {
    try {
      ...
      if (!_rlBuckets[ip]) _rlBuckets[ip] = [];
      _rlBuckets[ip] = _rlBuckets[ip].filter(t => now - t < windowMs);
      if (_rlBuckets[ip].length >= maxPerMinute) {
        return res.status(429).json({ error: 'Too many requests — please wait a minute and try again.' });
      }
      _rlBuckets[ip].push(now);
      next();
    } catch (e) { next(); }
  };
}
// prune the rate-limit map every 10 min so it can't grow unbounded
setInterval(() => {
  const now = Date.now();
  Object.keys(_rlBuckets).forEach(ip => {
    _rlBuckets[ip] = _rlBuckets[ip].filter(t => now - t < 60000);
    if (_rlBuckets[ip].length === 0) delete _rlBuckets[ip];
  });
}, 10 * 60 * 1000);
...
// appended to a file (best-effort; Render free tier filesystem is ephemeral).
const pilotSignups = [];
app.post('/api/pilot/signup', rateLimit(10), (req, res) => {
  try {
    const { name, email, company } = req.body || {};
    if (!name || !email) return res.status(400).json({ error: 'name and email required' });
    const entry = { name: String(name).slice(0,120), email: String(email).slice(0,160), company: String(company||'').slice(0,160), at: new Date().toISOString() };
    pilotSignups.push(entry);
    // Console log — visible in Render dashboard logs even after filesystem resets
    console.log('PILOT_SIGNUP', JSON.stringify(entry));
    // Best-effort file append
    try { fs.appendFileSync(path.join(os.tmpdir(), 'pilot_signups.jsonl'), JSON.stringify(entry) + '\n'); } catch(e) {}
    res.json({ ok: true });
...
// Clear all schedules + queue for a user (clean slate for testing)
app.post('/api/auto/clear', requireMember, (req, res) => {
  const email = ((req.body && req.body.email) || '').toLowerCase();
  if (!email) return res.status(400).json({ error: 'email required' });
  Object.keys(autoSchedules).forEach(k => { if ((autoSchedules[k].userEmail||'').toLowerCase() === email) delete autoSchedules[k]; });
  Object.keys(contentQueue).forEach(k => { if ((contentQueue[k].userEmail||'').toLowerCase() === email) delete contentQueue[k]; });
  saveAuto();
  res.json({ ok: true });
});
...
app.all('/api/auto/cron', requireMember, async (req, res) => {
  const now = Date.now();
...
app.get('/api/runway/balance', rateLimit(30), requireMember, async (req, res) => {
  try {
...
app.post('/api/brand/profile', rateLimit(30), requireMember, async (req, res) => {
  try {
...
app.post('/api/runway/generate-image', rateLimit(30), requireMember, async (req, res) => {
  try {
...
app.post('/api/runway/generate', rateLimit(30), requireMember, async (req, res) => {
  try {
...
app.post('/api/runway/stitch', rateLimit(30), requireMember, async (req, res) => {
...
app.post('/api/whiteboard/animate', rateLimit(30), requireMember, async (req, res) => {
...
app.post('/api/brand/extract-colors', rateLimit(30), requireMember, async (req, res) => {
...
app.post('/api/heygen/generate', rateLimit(30), requireMember, async (req, res) => {
  if (!HEYGEN_KEY) return res.status(400).json({ error: 'HeyGen not configured' });
  try {
...
app.post('/api/heygen/character', rateLimit(30), requireMember, async (req, res) => {
  if (!HEYGEN_KEY) return res.status(400).json({ error: 'HeyGen not configured' });
  try {
...
app.post('/api/heygen/talkingphoto', rateLimit(30), requireMember, async (req, res) => {
  if (!HEYGEN_KEY) return res.status(400).json({ error: 'HeyGen not configured' });
  try {
...
app.post('/api/productad/finalize', rateLimit(30), requireMember, async (req, res) => {
...
app.post('/api/spokesperson/finalize', rateLimit(30), requireMember, async (req, res) => {
  ...
  try {
    ...
    const vf = "scale=" + W + ":" + H + ":force_original_aspect_ratio=decrease,pad=" + W + ":" + H + ":(ow-iw)/2:(oh-ih)/2:color=0x0B1F3A,setsar=1,fps=30,format=yuv420p";
...
app.post('/api/stock/preview', rateLimit(30), requireMember, async (req, res) => {
  try {
...
app.post('/api/slides/animate', rateLimit(30), requireMember, async (req, res) => {
...
app.get('/api/voice/preview', rateLimit(30), requireMember, async (req, res) => {
  try {
    const { url } = req.query;
