...
app.post('/api/whiteboard/animate', async (req, res) => {
  ...
  try {
    ...
    console.log('Whiteboard v8.20.0:', numScenes, 'scenes, pythonReady=' + pythonReady);
...
`;
    ...
    const fileSize = fs.statSync(outputPath).size;
    outputStore[videoId] = { path:outputPath, size:fileSize, created:Date.now() };
    console.log('Whiteboard v8.20.0 ready:', fileSize, 'bytes, id:', videoId);
    res.json({ videoId, downloadUrl:'/api/video/'+videoId, size:fileSize, scenes:imageUrls.length });

  } catch(e) {
    console.error('Whiteboard v8.20.0 error:', e.message);
    res.status(500).json({ error: e.message });
  } finally {
    try { fs.rmSync(tempDir,{recursive:true,force:true}); } catch(e) {}
  }
...
app.post('/api/slides/animate', async (req, res) => {
  ...
  try {
    ...
    console.log('Slides v8.20.0:', slides.length, 'slides,', W+'x'+H, audioMode||'music', 'pythonReady='+pythonReady);
...
`);
    ...
    const fileSize = fs.statSync(outputPath).size;
    outputStore[videoId] = { path:outputPath, size:fileSize, created:Date.now() };
    console.log('Slides v8.20.0 ready:', fileSize, 'bytes, id:', videoId);
    ...
    console.error('Slides v8.20.0 error:', e.message);
    res.status(500).json({ error: e.message });
  } finally {
    try { fs.rmSync(tempDir,{recursive:true,force:true}); } catch(e) {}
  }
...
app.listen(PORT, function() {
  console.log('EnerStudio Backend v8.20.0 running on port ' + PORT);
